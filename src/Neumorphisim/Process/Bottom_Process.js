...
const Bottom_Process = ({x, y, width, height}) => {
  const border = rrect(rect(0, 250, width, 148), 0, 0);
  const container = rrect(rect(2, 252, width - 4, 148 - 4), 0, 0);
  ...
  return (
    <FitBox src={rect(0, 0, width, height)} dst={rect(x, y, width, height)}>
      ...
      <Group transform={[{translateY: 280}]}>
        ...
        <Group transform={[{translateX: width - 45}]} color="rgb(186,186,186)">
          <Path
            path={
